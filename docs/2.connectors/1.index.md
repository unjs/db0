...
connectorDependencies["mysql2"];
// { lib: { name: "mysql2", import: "mysql2/promise", version: "^3" } }
```

`name` is the npm package to install. `import` is the specifier the connector actually imports and is only set when it differs from `name` (a subpath such as `mysql2/promise`), so resolve the library from `dep.import ?? dep.name` when you load it yourself to pass back via the `lib` option.
