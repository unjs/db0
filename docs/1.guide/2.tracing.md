...
The wrapper delegates every call to the original instance, so `connector`, `dialect`, `disposed`, `getInstance()` and `dispose()` behave exactly as they do on an untraced database. Two things differ:
