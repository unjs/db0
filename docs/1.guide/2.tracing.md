...
Tracing relies on `node:diagnostics_channel`, which requires Node.js `20.16+` or `22.3+`. On runtimes where it is unavailable (e.g. Cloudflare Workers), `withTracing` returns the database unchanged and no events are emitted.
...
- `connector`: The name of the connector (e.g. `better-sqlite3`, `postgresql`).
